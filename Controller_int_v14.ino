...
const int SELECT_BUTTON = 0; // // Push switch built into the rotary encoder
const int LCD_CLK = 3;  // Connected to CLK on KY-040
const int LCD_DT = 4;  // Connected to DT on KY-040
const int INHALE_LED = 12;
const int EXHALE_LED = 13;
...
//const long int TICKS_PER_MINUTE = 60000;          // Debug only
...
void setup()
{
  ...
  pinMode (LCD_CLK, INPUT);               // Input from CLK of rotary encoder
  pinMode (LCD_DT, INPUT);               // Input from DT of rotary encoder
  pinALast = digitalRead(LCD_CLK);        // Remember the state of the rotary encoder CLK
...
float getKnob(int settingMin, int settingMax, float currentVal, float setStep, const String& units_str) {
  ...
  while (selectButtonData == HIGH) {
    ...
    aVal = digitalRead(LCD_CLK);
    if (aVal != pinALast) { // Means the knob is rotating
      // if the knob is rotating, we need to determine direction
      // We do that by reading pin B.
      if (digitalRead(LCD_DT) != aVal) {  // Means pin A Changed first - We're Rotating Clockwise
        bCW = true;
